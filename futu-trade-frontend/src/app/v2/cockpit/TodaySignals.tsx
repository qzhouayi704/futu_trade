...
import {
  lifecycleCounts, permissionText, signalHeadline, stageText,
...
type LifecycleFilter = "all" | "active" | "confirmed" | "invalidated";
...
function SignalDetails({ item }: { item: V2AlertPerformanceItem }) {
  return <details className="mt-1 text-[11px] text-muted-foreground"><summary className="cursor-pointer">阶段记录 · {item.alert_count}次</summary><div className="mt-1 space-y-1">
    {(["SETUP", "WATCHING", "CONFIRMED"] as const).map((stage) => {
      ...
      return point && <div key={stage}>{stageText[stage]} {signalClock(point.time)} / {point.price.toFixed(3)}</div>;
    })}
    <div className="max-w-52 break-words">首次原因：{candidateReasonText(item.reason_code)}</div>
    <div className="max-w-52 break-words">当前判断：{candidateReasonText(item.current_reason_code)}</div>
    <div className="max-w-52 break-words">基准时策略：{strategySourcesText(item.strategy_sources)}</div>
    <div className="max-w-52 break-words">当日曾命中：{strategySourcesText(item.ever_strategy_sources)}</div>
    <div className="max-w-52 break-words">当前策略：{strategySourcesText(item.current_strategy_sources)}</div>
    <div>当日最高阶段：{stageText[item.max_stage]}</div>
    <div>当前状态时间：{signalClock(item.current_state_time)}</div>
...
export function TodaySignals() {
  ...
  const strategySources = [...new Set(data?.items.flatMap((item) => item.strategy_sources) || [])].sort();
  const activeStrategySource = strategySource === "unclassified" || strategySources.includes(strategySource) ? strategySource : "";
  const needle = search.trim().toLocaleLowerCase();
  const rows = sortTodaySignals((data?.items || [])
    .filter((item) => !activeVersion || item.strategy_version === activeVersion)
    .filter((item) => !activeStrategySource
      || activeStrategySource === "unclassified" && !item.strategy_sources.length
      || item.strategy_sources.includes(activeStrategySource))
    .filter((item) => lifecycleFilter === "all"
      || lifecycleFilter === "active" && !["IDLE", "INVALIDATED"].includes(item.current_status)
      || lifecycleFilter === "confirmed" && item.current_status === "CONFIRMED"
      || lifecycleFilter === "invalidated" && item.current_status === "INVALIDATED")
      ...
      <div className="flex max-w-full items-center gap-2">
        <div className="flex min-w-0 flex-wrap border border-border bg-muted/25 p-0.5" role="group" aria-label="今日信号范围">
          {scopes.map((option) => <button key={option.id} type="button" aria-pressed={scope === option.id}
            onClick={() => { setScope(option.id); setVersion(""); setStrategySource(""); setLifecycleFilter("all"); }}
    ...
    {data && <>
      ...
      <div className="my-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex max-w-full flex-wrap items-center gap-2 text-xs">
          <select aria-label="今日信号当前状态" value={lifecycleFilter} onChange={(event) => setLifecycleFilter(event.target.value as LifecycleFilter)} className="h-8 max-w-full border border-border bg-background px-2">
            <option value="all">全部当前状态</option>
            <option value="active">仅当前有效</option>
            <option value="confirmed">仅当前确认</option>
            <option value="invalidated">仅已失效</option>
          </select>
          {(strategySources.length > 0 || data.items.some((item) => !item.strategy_sources.length)) && <select aria-label="今日信号策略来源" value={activeStrategySource} onChange={(event) => setStrategySource(event.target.value)} className="h-8 max-w-full border border-border bg-background px-2">
            <option value="">全部基准时策略</option>
            {strategySources.map((source) => <option key={source} value={source}>{strategySourceLabel(source)}</option>)}
            <option value="unclassified">基准时暂无策略提名</option>
          </select>}
