    ...
    async def read(self, alerts: list[dict], as_of: datetime) -> dict[str, TapePerformance]:
        ...
        for offset in range(0, len(requests), self.BATCH_SIZE):
            ...
            query = (
                ...
                "SELECT b.event_id,t.price,"
                "FIRST_VALUE(t.trade_time) OVER observed AS first_time,"
                "LAST_VALUE(t.trade_time) OVER observed AS last_time,"
                "LAST_VALUE(t.price) OVER observed AS last_price "
                ...
                "WINDOW observed AS (PARTITION BY b.event_id ORDER BY "
                f"{HK_TRADE_JD},t.id ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) "
                "SELECT event_id,MAX(last_price),MAX(price),MIN(price),"
                "MAX(first_time),MAX(last_time),COUNT(*) "
